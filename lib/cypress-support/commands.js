      ...
      cy.exec('node_modules/.bin/wp-cypress wp "wp-cypress-set-user --logout"').then(() => {
        ...
        cy.get('body').then((body) => {
          if (body.find('.jetpack-sso-toggle.wpcom').length > 0) {
             ...
             * If the link is visible click the link and continues the process, else stop clicking.
             * Because even if Jetpack SSO is installed there were instances where the login form is available but ` login using username and password` can be in hidden mode.
             */
            cy.get('.jetpack-sso-toggle.wpcom').then(($link) => {
              if ($link.is(':visible')) {
                $link.click()
              }
            })
